...
Replaces LK frame-to-frame tracking with map-point matching.

Pipeline each frame
───────────────────
1. ORB detect + compute in current frame
2. BFMatcher knnMatch frame descriptors → global map descriptors (ratio test)
3. solvePnPRansac on matched (world 3D, pixel 2D) pairs → camera pose
4. At each keyframe: unmatched ORB kps with valid depth → new map points

Saves map_v15.ply on exit.   Q to quit.
"""
...
MIN_MATCHES = 12
MAX_HAMMING = 60       # descriptor distance threshold for ratio-test survivors

KF_TRANS      = 0.15   # metres
KF_ROT_DEG    = 15.0
MIN_KF_FRAMES = 8
MAX_MAP_PTS   = 30_000
...
if not (0.0001 < depth_scale < 0.01):          # sanity: D435 should be ~0.001
...
# ── Global map ────────────────────────────────────────────────────────────────
_map_pts3d = []    # world-space positions  (list of np.ndarray shape (3,))
_map_des   = []    # ORB descriptors        (list of np.ndarray shape (32,))
_map_voxels = set()
...
def depth_at(depth_raw, u, v, win=3):
    """Median valid depth in a (2*win+1)² patch around (u, v)."""
    u0, v0 = int(u), int(v)
...
def lift_to_world(u, v, z, pose):
    """Pixel + depth → world 3D using camera pose."""
...
vis.create_window("map_v15 — ORB map tracking", width=900, height=650)
...
try:
    while True:
        ...
        # ── Match current ORB descriptors against global map ─────────────────
        # Relax thresholds when already lost — easier to recover with weaker matches
        ...
        if frames_since_kf >= MIN_KF_FRAMES:
            ...
            # When tracking: trigger on motion.  When lost: trigger on time alone
            # (pose is frozen so dist/rrot never change — time-based keeps map growing
            # into new view directions so tracking can recover after a turn).
            do_kf = (dist > KF_TRANS or rrot > KF_ROT_DEG) if mode == "map" \
                    else (frames_since_kf >= MIN_KF_FRAMES)
            if do_kf:
                added = 0
                if des is not None:
                    for i, k in enumerate(kps):
                ...
                last_kf_pose    = global_pose.copy()
                frames_since_kf = 0
                print(f"  [KF/{mode}]  f:{frame_idx}  map:{len(_map_pts3d)}  "
        ...
        cv2.imshow("map_v15 — tracking", dbg)
    ...
    if _trail_pts:
        np.save("trail_v15.npy", np.array(_trail_pts))
        print(f"[saved]  trail_v15.npy  —  {len(_trail_pts)} poses")
