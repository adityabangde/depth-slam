...
map_v6.py
---------
map_v5  +  2-D top-down visualisation instead of Open3D.

What changed vs v5
──────────────────
• Open3D removed entirely — much lighter, runs on Raspberry Pi 5.
• 3-D map is stored as a plain NumPy list + voxel set (same dedup logic).
• Live view is a 2-D bird's-eye OpenCV window:
    grey dots  — map points projected onto X-Z floor plane
    red  line  — camera trajectory
    green dot  — current camera position
    crosshair  — world origin
• POS_ALPHA added — adaptive position smoothing, kills rotation-induced spikes.
• Saves map_v6_pts.npy + map_v6_trajectory.npy on exit.
...
ORB_FEATURES  = 1000
RENDER_EVERY  = 3
CARVE_EVERY   = 10           # less frequent → faster playback
FREE_MARGIN   = 0.10
TRAJ_SMOOTH_WIN = 15         # running average window for live trajectory display
...
# Position smoothing — kills rotation-induced translation spikes
POS_ALPHA     = 0.5    # fixed fallback; adaptive version used when pose found


# ── 2-D canvas settings ────────────────────────────────────────────────────────
CANVAS_W  = 900          # pixels
CANVAS_H  = 900
MAP_SCALE = 100          # pixels per metre  (900px / 100 = 9 m view)
OX        = CANVAS_W // 2   # world origin in canvas pixels
OZ        = CANVAS_H // 2


def _w2px(x: float, z: float):
    """World (x, z) → canvas pixel (col, row).  Forward (Z+) = up on screen."""
    return int(OX + x * MAP_SCALE), int(OZ - z * MAP_SCALE)


# ── Hamming distance LUT ──────────────────────────────────────────────────────
...
# ── Map storage (replaces Open3D PointCloud) ──────────────────────────────────
# Preallocated numpy buffer — avoids repeated list→array conversion in carving.
_MAP_CAP      = 200_000
_map_buf      = np.zeros((_MAP_CAP, 3), dtype=np.float32)
_map_n        = 0
occupied: set = set()

def _map_pts() -> np.ndarray:
    """Return view of the filled portion of the buffer."""
    return _map_buf[:_map_n]

# Persistent canvas — map points drawn once and kept
...
def _draw_pts_on_canvas(pts: np.ndarray):
    """Draw new 3-D points onto the persistent canvas (X-Z projection)."""
    if len(pts) == 0:
        return
    px = (OX + pts[:, 0] * MAP_SCALE).astype(np.int32)
    pz = (OZ - pts[:, 2] * MAP_SCALE).astype(np.int32)
    valid = (px >= 0) & (px < CANVAS_W) & (pz >= 0) & (pz < CANVAS_H)
    map_canvas[pz[valid], px[valid]] = (140, 140, 140)


def _rebuild_canvas():
    """Rebuild canvas from scratch after carving removes points."""
    global map_canvas
    map_canvas = np.full((CANVAS_H, CANVAS_W, 3), 15, dtype=np.uint8)
    cv2.line(map_canvas, (OX, 0),  (OX, CANVAS_H), (45, 45, 45), 1)
    cv2.line(map_canvas, (0,  OZ), (CANVAS_W, OZ), (45, 45, 45), 1)
...
def render_2d(smooth_traj: list):
    """Compose the final display frame: map + smoothed trajectory + camera dot."""
...
# ── Main loop ─────────────────────────────────────────────────────────────────
print("\nMapping (2D view) — move camera slowly.  Q to quit.\n")
...
frames_since_kf = 0
frame_idx       = 0
trajectory      = []                        # raw positions — saved to file
smooth_traj     = []                        # running average — display only
...
try:
    while True:
        ...
        # ── Update pose (adaptive position smoothing) ─────────────────────────
        if new_pose is not None:
            velocity  = np.linalg.inv(global_pose) @ new_pose
            vel_trans = np.linalg.norm(velocity[:3, 3])
            # Adaptive alpha: small movement → strong smoothing (freeze position)
            #                 large movement → weak smoothing  (trust new pose)
        ...
        if frame_idx % RENDER_EVERY == 0:
            cv2.imshow("Map v6 — 2D top-down", render_2d(smooth_traj))

        # ── Debug overlay (camera feed) ───────────────────────────────────────
    ...
    if _map_n > 0:
        np.save("map_v6_pts.npy", _map_pts().copy())
    if trajectory:
        np.save("map_v6_trajectory.npy", np.array(trajectory))
    print(f"\n[saved] map_v6_pts.npy          — {_map_n:,} points")
    print(f"[saved] map_v6_trajectory.npy   — {len(trajectory)} poses")
    pipeline.stop()
