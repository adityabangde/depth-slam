...
map_v12.py
----------
map_v10  +  MPU-9250 yaw integration via ESP32 serial.

Yaw (heading) comes from the MPU-9250 magnetometer fused with gyro via
Madgwick filter running on ESP32.  Only yaw is taken from the IMU —
pitch/roll stay at zero (camera is roughly horizontal while walking).
PnP is still used for translation (X/Z position).

IMU wiring
──────────
  MPU-9250 → ESP32 GPIO 21 (SDA) / 22 (SCL) → USB serial to PC

Serial format from ESP32 (115200 baud):
  yaw_deg,timestamp_ms\\n
  e.g.  -45.231,12043

YAW_SIGN in settings: set to -1 if trail curves the wrong way on screen.

Saves map_v12_pts.npy + map_v12_trajectory.npy on exit.
Controls → Q to quit
"""
...
# ── Settings ──────────────────────────────────────────────────────────────────
PLAYBACK_FILE   = r"recordings\2026-04-13_18-20-50.bag"
...
# IMU — set IMU_CSV for playback, IMU_PORT for live camera
IMU_CSV    = r"recordings\2026-04-13_18-20-50_imu.csv"  # set to matching _imu.csv
IMU_PORT   = 'COM5'     # used only when PLAYBACK_FILE is empty (live mode)
IMU_BAUD   = 115200
YAW_SIGN   = -1         # flip to -1 if trail curves wrong way
...
# ── IMU ───────────────────────────────────────────────────────────────────────
def _yaw_R(yaw_deg):
    """Yaw-only rotation matrix (rotation around Y — vertical axis)."""
    a = math.radians(yaw_deg * YAW_SIGN)
...
class IMUPlayback:
    """Serves yaw from a pre-recorded CSV keyed by frame index."""
    def __init__(self, csv_path):
        ...
        try:
            with open(csv_path) as f:
                next(f)   # skip header
                for line in f:
                    idx, yaw = line.strip().split(',')
                    self._data[int(idx)] = float(yaw)
            self.ok = True
            print(f"[IMU]  loaded {len(self._data)} rows from {csv_path}")
        except Exception as e:
            print(f"[IMU]  CSV not loaded ({e}) — using visual rotation")
        self._frame = 0
    ...
    def get_yaw(self):
        return self._data.get(self._frame, 0.0)

    def get_R(self):
        return _yaw_R(self.get_yaw())


class IMULive:
    """Reads yaw from ESP32 serial in a background thread."""
    def __init__(self, port, baud=115200):
        ...
        try:
            ...
            self.ok = True
            print(f"[IMU]  live on {port}")
        except Exception as e:
            print(f"[IMU]  not connected ({e}) — using visual rotation")
...
# Choose playback or live based on whether PLAYBACK_FILE is set
if PLAYBACK_FILE:
    imu = IMUPlayback(IMU_CSV)
else:
    imu = IMULive(IMU_PORT, IMU_BAUD)
...
try:
    while True:
        ...
        frame_idx += 1
        imu.set_frame(frame_idx)          # sync IMU to current frame
        ...
        # ── Update pose ───────────────────────────────────────────────────────
        if new_pose is not None:
            # Replace PnP rotation with IMU yaw (magnetometer — no drift)
            if imu.ok:
                new_pose[:3,:3] = imu.get_R()
        ...
        if frame_idx % RENDER_EVERY == 0:
            cv2.imshow("Map v12 — 2D top-down", render_2d())
    ...
    if n_save > 0:
        np.save("map_v12_pts.npy", pts_save)
    if traj_save:
        np.save("map_v12_trajectory.npy", np.array(traj_save))

    print(f"\n[saved] map_v12_pts.npy         — {n_save:,} points")
    print(f"[saved] map_v12_trajectory.npy  — {len(traj_save)} poses")
    pipeline.stop()
