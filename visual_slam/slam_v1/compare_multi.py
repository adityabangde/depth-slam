...
compare_multi.py
----------------
Compare multiple bag recordings of the same path.

Phase 1  — first bag builds the map + trail 1
Phase 2+ — every other bag localises against that map (read-only)

All trails shown overlaid in final Open3D window.
Each trail gets a different colour.   Q to skip a bag.
"""
...
# ── Bag files ─────────────────────────────────────────────────────────────────
BAG_FILES = [
    r"recordings\2026-04-23_15-45-28.bag",
    r"recordings\2026-04-23_15-47-4.bag",
    r"recordings\2026-04-23_15-49-41.bag",
    r"recordings\2026-04-23_15-55-40.bag",
    r"recordings\2026-04-23_15-57-29.bag",
]
...
def pnp(pts3d, pts2d):
    ...
    ok, rvec, tvec, inliers = cv2.solvePnPRansac(
        ...
        K_MAT, DIST, iterationsCount=200, reprojectionError=4.0, confidence=0.99)
...
def run_bag(bag_path, label, allow_map_growth):
    name = os.path.basename(bag_path)
    print(f"\n{'─'*60}")
    print(f"  {label}  —  {name}")
    print(f"  map_growth={allow_map_growth}   map size={len(_map_pts3d)}")
    print(f"  Q to skip to next bag")
    print(f"{'─'*60}")
    ...
    try:
        while True:
            ...
            if not seeded:
                ...
                seeded = True
                print(f"  [seed]  map:{len(_map_pts3d)}")
                continue

            hamming_thresh = int(MAX_HAMMING * 1.3) if mode == "lost" else MAX_HAMMING
            min_match_req  = max(4, MIN_MATCHES // 2) if mode == "lost" else MIN_MATCHES
            ...
            if des is not None and len(_map_des) >= min_match_req:
                ...
                for pair in raw_m:
                    if len(pair) < 2:
                        continue
                    m, n = pair[0], pair[1]
                    if m.distance < RATIO_TEST * n.distance and m.distance < hamming_thresh:
            ...
            if allow_map_growth and mode == "map" and frames_since_kf >= MIN_KF_FRAMES:
                ...
                if dist > KF_TRANS or rrot > KF_ROT_DEG:
                    ...
                    last_kf_pose    = global_pose.copy()
                    frames_since_kf = 0
                    print(f"  [KF]  f:{frame_idx}  map:{len(_map_pts3d)}  +{added}  matched:{len(matched_3d)}")
            ...
            cv2.putText(dbg, f"{label}  mode:{mode}  map:{len(_map_pts3d)}  matched:{len(matched_3d)}  f:{frame_idx}",
                        (8, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (255, 255, 255), 1)
            cv2.putText(dbg, f"x:{global_pose[0,3]:.2f}  y:{global_pose[1,3]:.2f}  z:{global_pose[2,3]:.2f}",
                        (8, 44), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0, 220, 255), 1)

            cv2.imshow("compare_multi", dbg)
...
for i, bag in enumerate(BAG_FILES):
    if not os.path.exists(bag):
        print(f"[skip]  not found: {bag}")
        all_trails.append([])
        continue
    label          = f"BAG-{i+1}"
    allow_growth   = (i == 0)   # only first bag builds the map
    trail          = run_bag(bag, label, allow_growth)
    all_trails.append(trail)
    tag = os.path.basename(bag).replace(".bag", "")
    if trail:
        np.save(f"trail_multi_{i+1}_{tag}.npy", np.array(trail))
        print(f"[saved]  trail_multi_{i+1}_{tag}.npy")
...
if _map_pts3d:
    ...
    o3d.io.write_point_cloud("map_multi.ply", pcd)
    print(f"[saved]  map_multi.ply  ({len(_map_pts3d):,} points)")
...
# ── Final Open3D view ─────────────────────────────────────────────────────────
print("\nOpening Open3D window …  (close to exit)")

vis = o3d.visualization.Visualizer()
vis.create_window("Multi-bag comparison", width=1200, height=750)
...
for i, trail in enumerate(all_trails):
    ...
    ls.lines  = o3d.utility.Vector2iVector([[j, j+1] for j in range(len(pts)-1)])
    ls.colors = o3d.utility.Vector3dVector(np.tile(color, (len(pts)-1, 1)))
    vis.add_geometry(ls)

    sp = o3d.geometry.TriangleMesh.create_sphere(radius=0.07)
