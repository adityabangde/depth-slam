...
Changes from v16
────────────────
1. Ratio test  0.75 → 0.85   (accepts more matches)
2. MAX_HAMMING  60 → 80      (accepts weaker but still good descriptors)
3. MIN_MATCHES  12 → 8       (PnP needs fewer inliers to accept a pose)
4. RANSAC iterations 100 → 200  (handles extra outliers from relaxed thresholds)
   → typical match count goes from 12–15 up to 80–150
   → RANSAC inside solvePnPRansac handles the extra outliers automatically

Saves map_v17.ply and trail_v17.npy on exit.   Q to quit.
"""
...
MIN_MATCHES = 8            # v17: was 12
MAX_HAMMING = 80           # v17: was 60
RATIO_TEST  = 0.85         # v17: was 0.75
...
vis.create_window("map_v17 — ORB map tracking", width=900, height=650)
...
try:
    while True:
        ...
        cv2.imshow("map_v17 — tracking", dbg)
    ...
    if _trail_pts:
        np.save("trail_v17.npy", np.array(_trail_pts))
        print(f"[saved]  trail_v17.npy  —  {len(_trail_pts)} poses")
