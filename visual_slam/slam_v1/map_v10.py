...
map_v10.py
----------
map_v9  +  per-frame rotation cap to fix yaw drift during fast turns.

When LK drops tracks mid-turn, PnP can estimate a large spurious rotation.
All subsequent poses inherit that wrong orientation → 90° curve in the trail.
Fix: cap rotation change per frame using Rodrigues vector scaling.
...
Saves map_v10_pts.npy + map_v10_trajectory.npy on exit.
Controls → Q to quit
"""
...
FREE_MARGIN     = 0.10
TRAJ_SMOOTH_WIN = 15
MIN_ROT_TRACKS = 30        # freeze rotation update when fewer LK tracks survive (fast turn)
...
try:
    while True:
        ...
        # ── Update pose ───────────────────────────────────────────────────────
        if new_pose is not None:
            # Freeze rotation when too few tracks survive — fast turns drop LK points
            # and PnP gives an unreliable yaw estimate with sparse correspondences
            if n_tracks < MIN_ROT_TRACKS:
                new_pose[:3,:3] = global_pose[:3,:3]
    ...
    if n_save > 0:
        np.save("map_v10_pts.npy", pts_save)
    if traj_save:
        np.save("map_v10_trajectory.npy", np.array(traj_save))

    print(f"\n[saved] map_v10_pts.npy         — {n_save:,} points")
    print(f"[saved] map_v10_trajectory.npy  — {len(traj_save)} poses")
    pipeline.stop()
